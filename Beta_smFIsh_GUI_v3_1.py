    ...
    def segment_spts(self):
        ...
        if self.tabs_tot.currentIndex() == 1:
            ...
            try:
                ...
                hh                      =  self.frame_spts_segm_b.view.viewRange()
                self.frame_nucs_cntr_b  =  self.frame_spts_segm_b.currentIndex
                self.spts_segm_b        =  Spots3D_Segmentation_beta.Spots3D_Segmentation(self.spts_detect_b, self.raw_data.spts_b, self.int_thr_value_b, self.vol_thr_value_b, self.min_numb_z_value_b, self.sph_thr_value_b, self.segm_clstr_flag_b).spts_lbls   # need to define int_thr from the backgroung image
                # self.spts_segm        =  Spots3D_Segmentation.Spots3D_Segmentation(self.spts_detect.spts_detect)   # need to define int_thr from the backgroung image
                if self.rmv_central_flag  == 1:
...
class ChopStack(QtWidgets.QWidget):
    ...
    def __init__(self, raw_data):
        ...
        if hasattr(raw_data, 'spts_a') == True:
            frame_spts_raw_a.setImage(raw_data.spts_a)
        else:
            frame_spts_raw_a.setImage(np.zeros(raw_data.spts_b.shape, dtype=np.int8))
        ...
        if hasattr(raw_data, 'spts_b') == True:
            frame_spts_raw_b.setImage(raw_data.spts_b)
        else:
            frame_spts_raw_b.setImage(np.zeros(raw_data.spts_a.shape, dtype=np.int8))
